...
class TouchEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        ee_pos_rf, _ = self.robot.global_to_local_coordinate_frame(ee_pos)
        obj_pos_rf, _ = self.robot.global_to_local_coordinate_frame(obj_pos)
        diff_pos = obj_pos_rf - ee_pos_rf
        all_angles = self.robot.get_joint_angles(self.robot.controllable_joints)
        obs_jnt_angles = np.array([all_angles[2], np.sum(all_angles[3:7]), *all_angles[7:10]])
        observations = np.concatenate([ee_pos_rf, obj_pos_rf, diff_pos, obs_jnt_angles])
    ...
    def reset(self, seed=None, options=None):
        ...
        self.init_obj_pos, _ = self.object.get_base_pos_orient()
...
from tianshou.highlevel.trainer import EpochStopCallbackRewardThreshold, EpochStopCallback, TrainingContext

class EpochStopCallbackPatience(EpochStopCallback):
    ...
    def should_stop(self, mean_rewards: float, context: TrainingContext) -> bool: 
        ...
        if self.epochs_without_improvement >= self.patience:
            self.stop = True
            print("Training is stopped since reward doesn't improve in the last 10 epochs.")
...
def run_experiment():
    experiment = (
        ...
        .with_epoch_stop_callback(EpochStopCallbackPatience(15))
        .build()
    )
